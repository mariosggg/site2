 ...
 */
class Fana_Megamenu_Walker extends Walker_Nav_Menu {
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if (isset($item->mega_data) && !empty($item->mega_data)) {
            if (!empty($item->mega_data['icon'])) {
                ...
                $icon = fana_megamenu_get_icon_html($icon_class, $item->mega_data);
            }
        }
        ...
        $post_id = fana_megamenu_get_post_related_menu($item->ID);
    ...
    public function display_element($element, &$children_elements, $max_depth, $depth, $args, &$output) {
        ...
        $element->mega_data = fana_megamenu_get_item_data($element->ID);
    ...
    public function render_megamenu_elementor($item, $args, $depth) {
        ...
        if ( fana_elementor_activated() ) {
            ...
            $post_id = fana_wpml_object_id( $post_id, 'post', true, apply_filters( 'wpml_current_language', NULL ) );

            $content = Elementor\Plugin::instance()->frontend->get_builder_content_for_display($post_id, fana_get_elementor_css_print_method());
            ...
            if( fana_tbay_get_config('ajax_dropdown_megamenu', false) ) {  
