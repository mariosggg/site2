<?php
if (!defined('ABSPATH') || function_exists('Fana_Elementor_Widget_Base')) {
...
abstract class Fana_Elementor_Widget_Base extends Elementor\Widget_Base
{
    ...
    protected function get_view_template($tpl_slug, $tpl_name, $settings = [])
    {
        ...
        foreach ($templates as $template) {
            if (file_exists(FANA_THEMEROOT . '/' . $template)) {
        ...
        if ($located) {
            ...
            echo sprintf(__('Failed to load template with slug "%s" and name "%s".', 'fana'), $tpl_slug, $tpl_name);
        }
    }
    ...
    protected function register_controls_heading($condition = array(), $full = false)
    {
        ...
        if( $full ) {
            $this->add_control(
                'heading_list_style',
                [
                    'label'         => esc_html__('Heading Style', 'fana'),
    ...
    private function register_section_styles_icon($condition) {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__( 'Heading Icon', 'fana' ),
    ...
    private function register_section_styles_content($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Content', 'fana'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    private function register_section_styles_title($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Title', 'fana'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    private function register_section_styles_sub_title($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Sub Title', 'fana'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    private function register_section_styles_description($condition)
    {
        $this->start_controls_section(
            ...
            [
                'label' => esc_html__('Heading Description', 'fana'),
                'tab'   => Controls_Manager::TAB_STYLE,
                'condition' => $condition,
            ]
        );
    ...
    protected function get_available_menus_default()
    {
        ...
        if ($menus) {
            foreach ($menus as $menu) {
                $options[ fana_get_transliterate($menu->slug) ] = $menu->name;
            }
        }
    ...
    {
        return apply_filters('fana_get_template_product_grid', 'v1');
    }

    protected function get_template_product_vertical()
    {
        return apply_filters('fana_get_template_product_vertical', 'vertical');
    }

    protected function get_template_product()
    {
        return apply_filters('fana_get_template_product', 'v1');
    }

    protected function get_product_type()
    {
        $type = [
            'newest' => esc_html__('Newest Products', 'fana'),
            'on_sale' => esc_html__('On Sale Products', 'fana'),
            'best_selling' => esc_html__('Best Selling', 'fana'),
            'top_rated' => esc_html__('Top Rated', 'fana'),
            'featured' => esc_html__('Featured Product', 'fana'),
            'random_product' => esc_html__('Random Product', 'fana'),
        ];

        return apply_filters('fana_woocommerce_product_type', $type);
    }
    ...
    protected function get_product_categories($number = '')
    {
        ...
        if (!is_wp_error($category)) {
            foreach ($category as $category) {
                $results[fana_get_transliterate($category->slug)] = $category->name.' ('.$category->count.') ';
            }
        }
        return $results;
    }
    ...
    {
        $operator = [
            'AND' => esc_html__('AND', 'fana'),
            'IN' => esc_html__('IN', 'fana'),
            'NOT IN' => esc_html__('NOT IN', 'fana'),
        ];

        return apply_filters('fana_woocommerce_cat_operator', $operator);
    }

    protected function get_woo_order_by()
    {
        $oder_by = [
            'date' => esc_html__('Date', 'fana'),
            'title' => esc_html__('Title', 'fana'),
            'id' => esc_html__('ID', 'fana'),
            'popularity' => esc_html__('Popularity', 'fana'),
            'rand' => esc_html__('Random', 'fana'),
            'menu_order' => esc_html__('Menu Order', 'fana'),
        ];

        return apply_filters('fana_woocommerce_oder_by', $oder_by);
    }

    protected function get_woo_order()
    {
        $order = [
            'asc' => esc_html__('ASC', 'fana'),
            'desc' => esc_html__('DESC', 'fana'),
        ];

        return apply_filters('fana_woocommerce_order', $order);
    }
    ...
    protected function register_woocommerce_order()
    {
        $this->add_control(
            'orderby',
            [
                'label' => esc_html__('Order By', 'fana'),
                'type' => Controls_Manager::SELECT,
                'default' => 'date',
    ...
    protected function register_woocommerce_categories_operator()
    {
        ...
        $this->add_control(
            'categories',
            [
                'label' => esc_html__('Categories', 'fana'),
        ...
        $this->add_control(
            'cat_operator',
            [
                'label' => esc_html__('Category Operator', 'fana'),
                'type' => Controls_Manager::SELECT,
                'default' => 'IN',
    ...
    protected function list_controls_effects() {
        $options = [
            'no'                => 'No Effect',
            'zoom-in-1'         => 'Zoom In 1',
            'zoom-in-2'         => 'Zoom In 2',
