...
if (!function_exists('fana_tbay_woocommerce_get_two_product_thumbnail')) {
    function fana_tbay_woocommerce_get_two_product_thumbnail()
    {
        global $product;
        ...
        if (has_post_thumbnail()) {
            ...
            $output .= '<img src="'.wc_placeholder_img_src().'" alt="'. esc_attr__('Placeholder', 'fana'). '" class="'. esc_attr($class) .'" width="'. esc_attr($placeholder_width) .'" height="'. esc_attr($placeholder_height) .'" />';
        }
        return trim($output);
    }
}
...
if (!function_exists('fana_tbay_woocommerce_get_silder_product_thumbnail')) {
    function fana_tbay_woocommerce_get_silder_product_thumbnail()
    {
        global $product;
...
if (!function_exists('fana_product_block_image_class')) {
    function fana_product_block_image_class($class = '')
    {
...
if (!function_exists('fana_slick_carousel_product_block_image_class')) {
    function fana_slick_carousel_product_block_image_class($class = '')
    {
        $images_mode   = apply_filters('fana_woo_display_image_mode', 10, 2);
...
if (!function_exists('fana_tbay_product_class')) {
    function fana_tbay_product_class($class = array())
    {

        $class_array    = array();

        $class_varible  = fana_is_product_variable_sale();
