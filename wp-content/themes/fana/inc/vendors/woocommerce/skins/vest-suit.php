...
add_action('fana_woocommerce_group_buttons', 'fana_the_yith_wishlist', 10, 1);
add_action('fana_woocommerce_group_buttons', 'fana_the_yith_compare', 20, 1);
add_action('fana_woocommerce_group_buttons', 'woocommerce_template_loop_add_to_cart', 30, 1);
add_action('fana_woocommerce_group_quick_view', 'fana_the_quick_view', 10, 1);
...
if ( ! function_exists( 'fana_woocommerce_quantity_mode_remove_grid_block_add_to_cart' ) ) {
    function fana_woocommerce_quantity_mode_remove_grid_block_add_to_cart() {
        if( fana_is_woo_variation_swatches_pro() || fana_woocommerce_quantity_mode_active() ) {
            remove_action('fana_woocommerce_group_buttons', 'woocommerce_template_loop_add_to_cart', 30);
        }
    }
