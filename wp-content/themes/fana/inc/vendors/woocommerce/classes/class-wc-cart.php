...
if (! class_exists('Fana_Cart')) :
    ...
    class Fana_Cart
    {
        ...
        {
            if (! isset(self::$instance) && ! (self::$instance instanceof Fana_Cart)) {
                self::$instance = new Fana_Cart();
            }
        ...
        public function __construct()
        {
            ...
            add_filter('fana_cart_position', array( $this, 'woocommerce_cart_position'), 10, 1);
            ...
            add_filter('fana_show_cart_mobile', array( $this, 'show_cart_mobile'), 10, 1);
        ...
        public function woocommerce_cart_position()
        {
            ...
            $position = fana_tbay_get_config('woo_mini_cart_position', 'popup');

            $position = (isset($_GET['ajax_cart'])) ? $_GET['ajax_cart'] : $position;

            $position =  (!in_array($position, $position_array)) ? fana_tbay_get_config('woo_mini_cart_position', 'popup') : $position;
        ...
        {
            if (isset($_REQUEST['fana_buy_now']) && $_REQUEST['fana_buy_now'] == true) {
                return __return_empty_string();
            }

            if (wp_is_mobile() && ! intval(fana_tbay_get_config('enable_buy_now', true))) {
                return __return_empty_string();
            } else {
                return $message;
            }
        ...
        {
            $active = fana_tbay_get_config('enable_add_cart_mobile', false);
        ...
        {
            $class = '';
            $active = apply_filters('fana_show_cart_mobile', 10, 2);
        ...
        public function subtotal_free_shipping( $return = false ) {
            if ( WC()->cart->is_empty() || !fana_tbay_get_config('show_cart_free_shipping', false) ) {
				return;
			}
				...
				if ( is_array( $min_amounts ) && $min_amounts ) {
					...
					if ( $current > 0 ) {
						...
						if ( $current < $min_amount ) {
                            ...
                            sprintf(
                                wp_kses(__('Spend %s more to reach <strong>FREE SHIPPING!</strong> <br /><span class="hide-in-cart">to add more products to your cart and receive free shipping for order %s.</span>', 'fana'), $allowed_html),
                                wc_price($spend),
                                wc_price($min_amount)
                            ) . 
                            '</div>';
                            ...
                            $content .= '<div class="tbay-total-condition-desc">';
                            $content .= sprintf(
                                esc_html__("Congratulations! You get free shipping with your order greater %s.", 'fana'),
                                wc_price($min_amount)
                            );
                            $content .= '</div>';
                            $content .= '</div>';
...
if (!function_exists('fana_cart')) {
    function fana_cart()
    {
        return Fana_Cart::getInstance();
    }
    fana_cart();
}
