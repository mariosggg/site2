...
if ( class_exists('WC_AJAX') ) :
    class FANA_WC_AJAX extends WC_AJAX
    {

        /**
         * Hook in ajax handlers.
         */
        public static function fana_init()
        {
            add_action('init', array(__CLASS__, 'define_ajax'), 0);
            add_action('template_redirect', array(__CLASS__, 'do_wc_ajax'), 0);
            
            self::fana_add_ajax_events();
        }

        /**
         * Hook in methods - uses WordPress ajax handlers (admin-ajax).
         */
        public static function fana_add_ajax_events()
        {
            /**
             * Register ajax event
             */
            $ajax_events = array(
                'fana_quantity_mini_cart',
                'fana_product_remove',
                'fana_products_categories_tab_shortcode',
                'fana_products_tab_shortcode',
                'fana_products_list_ajax',
                'fana_products_grid_ajax', 
                'fana_single_add_to_cart', 
                'fana_popup_variation_name', 
            );

            if ( defined('YITH_WCWL') ) {
                $ajax_events[] = 'fana_update_wishlist_count';
            }

            if ( fana_tbay_get_config('enable_quickview', true) ) {
                $ajax_events[] = 'fana_quickview_product';
            }
        ...
        public static function fana_quantity_mini_cart() { 
            check_ajax_referer( 'fana-minicartquantity-nonce', 'security' );
        ...
        public static function fana_product_remove() {
            check_ajax_referer( 'fana-productremove-nonce', 'security' );
        ...
        public static function fana_update_wishlist_count() {  
            check_ajax_referer( 'fana-wishlistcount-nonce', 'security' );
        ...
        public static function fana_products_categories_tab_shortcode() {
            if ( ! empty( $_POST['atts'] ) ) {
                check_ajax_referer( 'fana-productscategoriestab-nonce', 'security' );

                ob_start();

                $atts               = fana_clean( $_POST['atts'] );  
                $categories         = fana_clean( $_POST['value'] );
                $atts['categories'] = $categories;
    
                $data = fana_elementor_products_ajax_template( $atts );
        ...
        public static function fana_products_tab_shortcode() {
            if ( ! empty( $_POST['atts'] ) ) {
                check_ajax_referer( 'fana-productstab-nonce', 'security' );

                ob_start();

                $atts                   = fana_clean( $_POST['atts'] );
                $product_type           = fana_clean( $_POST['value'] );
        ...
        public static function fana_products_grid_ajax() {
            check_ajax_referer( 'fana-productsgrid-nonce', 'security' );

            // prepare our arguments for the query
            $args = json_decode(stripslashes($_POST['query']), true);
            
            fana_order_by_query($args['orderby'], $args['order']);
        ...
        public static function fana_products_list_ajax() {
            check_ajax_referer( 'fana-productslist-nonce', 'security' );
        ...
        public static function fana_quickview_product()
        {     
            ...
            foreach ($attributes as $attribute) {
                ...
                if (isset($variation[$taxonomy])) {
                    ...
                    if ($attribute['is_taxonomy']) {
                        // Don't use wc_clean as it destroys sanitized cfanacters
        ...
        public static function fana_single_add_to_cart() {
            /**
             * Clear Old Notices
             */
            wc_clear_notices();
            ...
            if (isset($_REQUEST['add-to-cart']) && is_numeric(wp_unslash($_REQUEST['add-to-cart']))) {
                 ...
                 */
                    ...
                    $woo_mess = empty($woo_mess) ? '<div class="woocommerce-message text-center" role="alert">' . esc_html__('Product added to cart successfully!', 'fana') . '</div>' : $woo_mess;
             ...
             */
                ...
                if (!isset($_REQUEST['product_id']) || !is_numeric(wp_unslash($_REQUEST['product_id']))){
                    wc_add_notice(esc_html__('Sorry, Product is not existing.', 'fana'), 'error');
                    wp_send_json(array(
                        'error' => true,
                        'message' => wc_print_notices(true)
                    ));
                ...
                if (!$error && $validate_attr['validate'] && $passed_validation && 'publish' === $product_status && WC()->cart->add_to_cart($product_id, $quantity, $variation_id, $variation)) {
                    ...
                    if (isset($validate_attr['missing_attributes'])) {
                        ...
                        wc_add_notice( _n('%s is a required field', '%s are required fields', $number, 'fana'), 'error');
                    } else {
                        wc_add_notice(esc_html__('Sorry, Maybe product empty in stock.', 'fana'), 'error');
                    }
        ...
        public static function fana_popup_variation_name() {
            if (!empty($_POST['variation_id'])) {    
                check_ajax_referer( 'fana-popupvariationname-nonce', 'security' );
    ...
    if (isset($_REQUEST['wc-ajax'])) {
        add_action('init', 'fana_init_wc_ajax');
        if (!function_exists('fana_init_wc_ajax')) :
            function fana_init_wc_ajax() {
                FANA_WC_AJAX::fana_init();
            }
        endif;
    }
endif;