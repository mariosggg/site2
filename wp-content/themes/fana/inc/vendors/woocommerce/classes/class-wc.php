<?php
if (! defined('ABSPATH') || !fana_woocommerce_activated()) {
    exit;
}

if (! class_exists('Fana_WooCommerce')) :


    class Fana_WooCommerce
    {
        public static $instance;

        public static function getInstance()
        {
            if (! isset(self::$instance) && ! (self::$instance instanceof Fana_WooCommerce)) {
                self::$instance = new Fana_WooCommerce();
            }
        ...
        private function init_hooks()
        {
            ...
            add_filter('fana_woo_pro_des_image', array( $this, 'shop_des_image_active'), 10, 1);
            ...
            add_action( 'fana_woocommerce_after_product_thumbnails', array( $this, 'quick_view_view_details_btn'), 10 );

            add_filter('fana_tbay_woocommerce_content_class', array( $this, 'woocommerce_content_class'), 10);
            ...
            add_action('fana_woocommerce_before_quick_view', array( $this, 'remove_quickview_hook'), 10);
            add_action('fana_woocommerce_after_quick_view', array( $this, 'add_quickview_hook'), 10);

            add_action('fana_tbay_after_shop_loop_item_title', array( $this, 'catalog_mode_remove_shop_loop_item_hook'), 10);
            ...
            add_filter('fana_enable_variation_selector', array( $this, 'enable_variation_selector'), 10);


            /*Show Quantity on mobile*/
            add_filter('fana_show_quantity_mobile', array( $this, 'show_quantity_mobile'), 10, 1);
            ...
            if ( fana_nextend_social_login_activated() ) {
            ...
            add_filter('fana_get_filter_title_mobile', array( $this, 'get_title_mobile'), 10, 1);
        ...
        public function woocommerce_scripts()
        {
            ...
            wp_enqueue_script('fana-woocommerce', FANA_SCRIPTS . '/woocommerce' . $suffix . '.js', array( 'fana-script' ), FANA_THEME_VERSION, true);

            wp_register_script('jquery-onepagenav', FANA_SCRIPTS . '/jquery.onepagenav' . $suffix . '.js', array( 'fana-script' ), '3.0.0', true);
        }
        ...
        public function compare_styles()
        {
            ...
            wp_enqueue_style('fana-font-tbay-custom');

            fana_tbay_fonts_url();

            wp_enqueue_style('fana-template');
            
            wp_enqueue_style('fana-skin');
        ...
        public function body_woo_class($classes)
        {
            ...
            $class  = fana_add_cssclass('woocommerce', $class);

            if (is_cart() && WC()->cart->is_empty()) {
                $class = fana_add_cssclass('empty-cart', $class);
            }
        ...
        public function widgets_init()
        {
            register_sidebar(array(
                'name'          => esc_html__('Product Archive Sidebar Top', 'fana'),
                'id'            => 'product-top-archive',
                'description'   => esc_html__('Add widgets here to appear in only shop page.', 'fana'),
            ...
            register_sidebar(array(
                'name'          => esc_html__('Product Archive Sidebar Bottom', 'fana'),
                'id'            => 'product-bottom-archive',
                'description'   => esc_html__('Add widgets here to appear in bottom product archive.', 'fana'),
                'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                'after_widget'  => '</aside>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
            ...
            register_sidebar(array(
                'name'          => esc_html__('Product Archive Sidebar', 'fana'),
                'id'            => 'product-archive',
                'description'   => esc_html__('Add widgets here to appear in Product archive left, right sidebar.', 'fana'),
                'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                'after_widget'  => '</aside>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
            ...
            register_sidebar(array(
                'name'          => esc_html__('Product Single Sidebar', 'fana'),
                'id'            => 'product-single',
                'description'   => esc_html__('Add widgets here to appear in Product single left, right sidebar.', 'fana'),
                'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                'after_widget'  => '</aside>',
                'before_title'  => '<h2 class="widget-title">',
                'after_title'   => '</h2>',
        ...
        public function woocommerce_content_class($class)
        {
            ...
            if (!isset($_GET['product_'.$page.'_layout'])) {
                $class .= ' '.fana_tbay_get_config('product_'.$page.'_layout');
            } else {
        ...
        {
            $class = '';
            $active = fana_catalog_mode_active();
        ...
        public function remove_quickview_hook()
        {
            ...
            remove_action('woocommerce_single_product_summary', 'fana_product_popup_group_buttons', 35);

            /*Add custom html before, after button add to cart*/
            remove_action('woocommerce_before_add_to_cart_form', 'fana_html_before_add_to_cart_button', 10);
            remove_action('woocommerce_after_add_to_cart_form', 'fana_html_after_add_to_cart_button', 99);

            /*Add custom html before, after inner product summary*/
            remove_action('woocommerce_single_product_summary', 'fana_html_before_inner_product_summary', 1);
            remove_action('woocommerce_single_product_summary', 'fana_html_after_inner_product_summary', 99);
            remove_action('fana_woocommerce_single_product_summary_left', 'fana_html_after_inner_product_summary', 99);

            remove_action('woocommerce_before_single_product', 'fana_html_before_product_summary', 5);
            remove_action('woocommerce_after_single_product', 'fana_html_after_product_summary', 5);
        }
        ...
        public function add_compare_styles()
        {
            ...
            if( !$show_typography ) {
                wp_enqueue_style( 'sfufutura' );
                wp_enqueue_style( 'ivymode' );
            }

            wp_enqueue_style( 'fana-style' );
            wp_enqueue_style( 'fana-theme-fonts' );
        }

        public function enable_variation_selector()
        {
            $active = fana_tbay_get_config('enable_variation_swatch', false);
        ...
        {
            $active = fana_tbay_get_config('enable_quantity_mobile', false);
        ...
        {
            $class = '';
            $active = apply_filters('fana_show_quantity_mobile', 10, 2);
            if (isset($active) && $active) {
                $class = 'tbmb-quantity';
            }
        ...
        {
            $active = fana_tbay_get_config('show_woocommerce_password_strength', true);
        ...
        public function yith_add_wcwl_link_my_account($items)
        {
            ...
            $items[$slug]                       =   esc_html__('My Wishlist', 'fana');
            $items['edit-address']              =   esc_html__('Addresses', 'fana');
            $items['payment-methods']           =   esc_html__('Payment methods', 'fana');
            $items['edit-account']              =   esc_html__('Account details', 'fana');
            $items['customer-logout']           =   esc_html__('Logout', 'fana');
        ...
        {
            $format                 =  fana_tbay_get_config('sale_tags', 'custom');
            $enable_label_featured  =  fana_tbay_get_config('enable_label_featured', true);

            if ($format == 'custom') {
                $format = fana_tbay_get_config('sale_tag_custom', '-{percent-diff}%');
            }
            ...
            if ($featured && $enable_label_featured) {
                $return_content  = '<span class="featured">'. fana_tbay_get_config('custom_label_featured', esc_html__('Hot', 'fana')) .'</span>';
            }


            if (!empty($product) && $product->is_type('variable')) {
                $default_attributes = fana_get_default_attributes($product);
                $variation_id = fana_find_matching_product_variation($product, $default_attributes);
                ...
                if (!empty($variation_id)) {
                    ...
                    $percentage = '<span class="saled">'. esc_html__('Sale', 'fana') . '</span>';
                }
        ...
        public function only_feature_product_label()
        {
            ...
            if ($featured) {
                ...
                if ($featured && $enable_label_featured) {
                    $return_content  .= '<span class="only-featured onsale"><span class="featured">'. fana_tbay_get_config('custom_label_featured', esc_html__('Hot', 'fana')) .'</span></span>';
        ...
        public function quick_view_view_details_btn( ) {
			...
			echo '<div class="details-btn-wrapper"><a class="view-details-btn" href="'. esc_url($permalink) .'">'. esc_html__('View details', 'fana') .'</a></div>';
        }
        

        public function quick_view_scripts()
        {
            if (!fana_tbay_get_config('enable_quickview', true)) {
                return; 
            }
        ...
        {
            if ( fana_nextend_social_login_activated() ) {
                echo '<div class="social-log"><span>'. esc_html__('Or connect with', 'fana') .'</span></div>';
            }
        }
        ...
        {
            if ( fana_nextend_social_login_activated() ) {
                echo '<div class="social-log"><span>'. esc_html__('Or login with', 'fana') .'</span></div>';
            }
        }
        ...
        public function show_product_outstock_flash_html($html)
        {
            ...
            if ($product->is_type('simple')) {
                if ($product->is_on_sale() &&  ! $product->is_in_stock()) {
                    $return_content .= '<span class="out-stock out-stock-sale"><span>'. esc_html__('Out of stock', 'fana') .'</span></span>';
                } elseif (! $product->is_in_stock()) {
                    $return_content .= '<span class="out-stock"><span>' . esc_html__('Out of stock', 'fana') .'</span></span>';
                }
            }
        ...
        {
            $columns = fana_tbay_get_config('number_product_thumbnail', 4);
        ...
        {
            $pagination_style = (isset($_GET['pagination_style'])) ? $_GET['pagination_style'] : fana_tbay_get_config('product_pagination_style', 'number');
        ...
        public function get_title_mobile($title)
        {
            if (is_author()) {
                ...
                $title = esc_html__('Tagged: "', 'fana'). single_tag_title('', false) . '"';
        ...
        {
            $active = fana_tbay_get_config('pro_des_image_product_archives', false);
...
function Fana_WooCommerce()
{
    return Fana_WooCommerce::getInstance();
}

// Global for backwards compatibility.
$GLOBALS['Fana_WooCommerce'] = Fana_WooCommerce();