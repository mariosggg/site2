...
if ( !function_exists('fana_tbay_fonts_url') ) {
	function fana_tbay_fonts_url() {  
        ...
        if ( $show_typography && $font_source == "2" && !empty($font_google_code)) {
            ...
            wp_enqueue_style('ivymode', FANA_STYLES . '/ivymode.css', array(), '1.0');
        }
	}
	add_action('wp_enqueue_scripts', 'fana_tbay_fonts_url');
}

if ( !function_exists('fana_custom_slick_prev') ) {
    function fana_custom_slick_prev( $icon ) {
        $icon = '<i class="tb-icon tb-icon-arrow-left"></i>';
        return $icon;
    }
    add_filter( 'fana_slick_prev', 'fana_custom_slick_prev', 10, 1 );
}

if ( !function_exists('fana_custom_slick_next') ) {
    function fana_custom_slick_next( $icon ) {
        $icon = '<i class="tb-icon tb-icon-arrow-right"></i>';
        return $icon;
    }
    add_filter( 'fana_slick_next', 'fana_custom_slick_next', 10, 1 );
}
...
if (! function_exists('fana_post_archive_the_read_more')) {
    function fana_post_archive_the_read_more( $text = '' )
    {
        ...
        ?>
			<div class="more">
				<a href="<?php the_permalink(); ?>" class="readmore" title="<?php echo trim($custom_readmore); ?>"><span class="text"><i class="tb-icon tb-icon-arrow-right"></i><?php echo trim($custom_readmore); ?></span></a>
			</div>
		<?php
    }
}