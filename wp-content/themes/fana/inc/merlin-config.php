...
require_once(get_parent_theme_file_path(FANA_INC . '/merlin-data/elementor.php'));

class Fana_Merlin_Config
{
    private $config = [];
    ...
    private function init()
    {
        $wizard = new Merlin(
            ...
            $strings = array(
                'admin-menu'          => esc_html__('Theme Setup', 'fana'),

                /* translators: 1: Title Tag 2: Theme Name 3: Closing Title Tag */
                'title%s%s%s%s'       => esc_html__('%1$s%2$s Themes &lsaquo; Theme Setup: %3$s%4$s', 'fana'),
                'return-to-dashboard' => esc_html__('Return to the dashboard', 'fana'),
                'ignore'              => esc_html__('Disable this wizard', 'fana'),

                'btn-skip'                 => esc_html__('Skip', 'fana'),
                'btn-next'                 => esc_html__('Next', 'fana'),
                'btn-start'                => esc_html__('Start', 'fana'),
                'btn-no'                   => esc_html__('Cancel', 'fana'),
                'btn-plugins-install'      => esc_html__('Install', 'fana'),
                'btn-child-install'        => esc_html__('Install', 'fana'),
                'btn-content-install'      => esc_html__('Install', 'fana'),
                'btn-import'               => esc_html__('Import', 'fana'),
                'btn-license-activate'     => esc_html__('Activate', 'fana'),
                'btn-license-skip'         => esc_html__('Later', 'fana'),

                /* translators: Theme Name */
                'license-header%s'         => esc_html__('Activate %s', 'fana'),
                /* translators: Theme Name */
                'license-header-success%s' => esc_html__('%s is Activated', 'fana'),
                /* translators: Theme Name */
                'license%s'                => esc_html__('Enter your license key to enable remote updates and theme support.', 'fana'),
                'license-label'            => esc_html__('License key', 'fana'),
                'license-success%s'        => esc_html__('The theme is already registered, so you can go to the next step!', 'fana'),
                'license-json-success%s'   => esc_html__('Your theme is activated! Remote updates and theme support are enabled.', 'fana'),
                'license-tooltip'          => esc_html__('Need help?', 'fana'),

                /* translators: Theme Name */
                'welcome-header%s'         => esc_html__('Welcome to %s', 'fana'),
                'welcome-header-success%s' => esc_html__('Hi. Welcome back', 'fana'),
                'welcome%s'                => esc_html__('This wizard will set up your theme, install plugins, and import content. It is optional & should take only a few minutes.', 'fana'),
                'welcome-success%s'        => esc_html__('You may have already run this theme setup wizard. If you would like to proceed anyway, click on the "Start" button below.', 'fana'),

                'child-header'         => esc_html__('Install Child Theme', 'fana'),
                'child-header-success' => esc_html__('You\'re good to go!', 'fana'),
                'child'                => esc_html__('Let\'s build & activate a child theme so you may easily make theme changes.', 'fana'),
                'child-success%s'      => esc_html__('Your child theme has already been installed and is now activated, if it wasn\'t already.', 'fana'),
                'child-action-link'    => esc_html__('Learn about child themes', 'fana'),
                'child-json-success%s' => esc_html__('Awesome. Your child theme has already been installed and is now activated.', 'fana'),
                'child-json-already%s' => esc_html__('Awesome. Your child theme has been created and is now activated.', 'fana'),

                'plugins-header'         => esc_html__('Install Plugins', 'fana'),
                'plugins-header-success' => esc_html__('You\'re up to speed!', 'fana'),
                'plugins'                => esc_html__('Let\'s install some essential WordPress plugins to get your site up to speed.', 'fana'),
                'plugins-success%s'      => esc_html__('The required WordPress plugins are all installed and up to date. Press "Next" to continue the setup wizard.', 'fana'),
                'plugins-action-link'    => esc_html__('Advanced', 'fana'),

                'import-header'      => esc_html__('Import Content', 'fana'),
                'import'             => esc_html__('Let\'s import content to your website, to help you get familiar with the theme.', 'fana'),
                'import-action-link' => esc_html__('Advanced', 'fana'),

                'ready-header'      => esc_html__('All done. Have fun!', 'fana'),

                /* translators: Theme Author */
                'ready%s'           => esc_html__('Your theme has been all set up. Enjoy your new theme by %s.', 'fana'),
                'ready-action-link' => esc_html__('Extras', 'fana'),
                'ready-big-button'  => esc_html__('View your website', 'fana'),
                'ready-link-1'      => sprintf('<a href="%1$s" target="_blank">%2$s</a>', 'https://tickets.thembay.com/', esc_html__('Ticket System', 'fana')),
                'ready-link-2'      => sprintf('<a href="%1$s">%2$s</a>', 'https://thembay-docs.gitbook.io/fana/', esc_html__('Documentation', 'fana')),
                'ready-link-3'      => sprintf('<a href="%1$s">%2$s</a>', 'https://www.youtube.com/c/thembay/', esc_html__('Video Tutorials', 'fana')),
                'ready-link-4'      => sprintf('<a href="%1$s">%2$s</a>', 'https://forums.thembay.com/', esc_html__('Forums', 'fana')),
            )
        );
    }
...
- - - - - - - - - - - - - - - - - - - - */";
    ...
    public function after_import_setup($selected_import)
    {
        ...
        $check_oneclick  = get_option('fana_check_oneclick', []);
        ...
        if (! isset($check_oneclick[ $selected_import['home'] ]) || apply_filters('fana_reset_import_rev_sliders', false)) {
            $check_oneclick[ $selected_import['home'] ] = true;
            $this->import_revslider($selected_import['rev_sliders']);
            update_option('fana_check_oneclick', $check_oneclick);
        }
    ...
    private function update_option_woof() {
        ...
        if (class_exists('YITH_WCBR')) {
            ...
            $settings['custom_tax_label']['yith_product_brand'] = esc_html__('Brands', 'fana');
        }
    ...
    private function import_files_skin_vendors() {
		$data = new Fana_Merlin_Elementor;
        ...
        switch ($prefix) {
            ...
            case 'mvx':
                return $data->import_files_demo_mvx();
    ...
    public function import_files()
    {
        ...
        $demo_vest 	    = $data->import_files_demo_vest();
        $demo_bikini 	= $data->import_files_demo_bikini();
        $demo_sport 	= $data->import_files_demo_sport();
        $demo_kids 	    = $data->import_files_demo_kids();
        $demo_dark 	    = $data->import_files_demo_dark();
        $vendors 	    = $this->import_files_skin_vendors();

		return array_merge($vendors, $demo_dark, $demo_vest, $demo_bikini, $demo_sport, $demo_kids, $demo_rtl);
    }
}

return new Fana_Merlin_Config();
